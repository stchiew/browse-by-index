...
export class SPService {
  ...
  public async getItems(xxtermsetID: string): Promise<IFileItem[]> {
    ...
    items.forEach((i) => {
      ...
      if (Array.isArray(i[this.fieldName])) {
        ...
        file.termGuid = i[this.fieldName]
          ? [i[this.fieldName].TermGuid.toLowerCase()]
          : [""];
        file.taxValue = i[this.fieldName]
          ? [
            `${i[this.fieldName].Label.toLowerCase()}|${i[
              this.fieldName
            ].TermGuid.toLowerCase()}`,
          ]
          : [""];
      }
      files.push(file);
  ...
  public async updateTaxonomyItemByAdd(
    file: IFileItem,
    fieldName: string,
    newTaxonomyValue: string
  ) {
  ...
  public async updateTaxonomyItemByReplace(
    file: IFileItem,
    fieldName: string,
    newTaxonomyValue: string
  ) {
  ...
  public async newTaxonomyItemByCopy(
    file: IFileItem,
    fieldName: string,
    newTaxonomyValue: string
  ): Promise<IFileItem> {
    ...
    const destinationUrl = decodeURI(fileUrl.pathname).replace(
      file.title,
      newFilename
    );
    await sp.web
      .getFileByServerRelativePath(decodeURI(fileUrl.pathname))
      .copyByPath(destinationUrl, false, true);
    const newFileItemPromise = await sp.web
      .getFileByServerRelativePath(destinationUrl)
      .getItem();
  ...
  public async newTaxonomyItemByUpload(
    file: any,
    fieldName: string,
    newTaxonomyValue: string
  ): Promise<IFileItem> {
    const libraryRoot = await sp.web.lists
      .getByTitle(this.listName)
      .rootFolder.get();
    const result = await sp.web
      .getFolderByServerRelativeUrl(libraryRoot.ServerRelativeUrl)
      .files.add(file.name, file, true);
    const fileNameParts = result.data.Name.split(".");
    const newFileItemPromise = await sp.web
      .getFileByServerRelativePath(result.data.ServerRelativeUrl)
      .getItem();
    const newFileItem = await newFileItemPromise.get();
